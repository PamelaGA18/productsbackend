...
export const createOrder = async (req, res) => {
  try {
    const { items, paymentMethod, subTotal, iva, total, totalProducts } =
      req.body;
      ...
      if (!product || product.quantity < item.quantity) {
        if (!product) {
          return res.status(400).json({
            message: `Producto ${item.productId} no encontrado en la base de datos`,
          });
        } else {
          return res.status(400).json({
      ...
      await Promise.all(
        order.items.map(async (item) => {
          await Products.findByIdAndUpdate(item.productId, {
            $inc: {
              quantity: item.quantity,
            }, // Sumar de vuelta al stock los productos cancelados
          });
        })
      );
    }
    ...
    try {
        ...
        if (!order) //No se encontró la orden
            return res.status(404)
                .json({
                    message: ['Error, no se encontró la orden']
                })
...
export const deleteOrder = async (req, res) => {
    try {
        ...
        if (!order) //No se encontró la orden
            return res.status(404)
                .json({
                    message: ['Error, no se encontró la orden para eliminar']
                })

        if (order.status !== 'cancelled')
            return res.status(400)
                .json({
                    message: ['Error, solo se pueden eliminar órdenes canceladas']
                });
